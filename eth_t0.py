...
for i in range(32):
    for j in range(32):
        if ((angle[i]-np.pi)*(angle[j]-np.pi) >= 0):
            g[i][j] = lam/(2*delta)*np.exp(-(angle[i]-angle[j])**2/(delta**2))
        elif (3/2*np.pi>angle[i]>np.pi and np.pi>angle[j]>np.pi/2):
            g[i][j] = lam/(2*delta)*np.exp(-(angle[i]-angle[j])**2/(delta**2))
        elif (np.pi>angle[i]>np.pi/2 and 3/2*np.pi>angle[j]>np.pi):
            g[i][j] = lam/(2*delta)*np.exp(-(angle[i]-angle[j])**2/(delta**2))
        else:
            g[i][j] = lam/(2*delta)*np.exp(-(angle[i]-angle[j] -
                                             np.sign(angle[i]-angle[j])*2*np.pi)**2/(delta**2))

''' 
f, ax= plt.subplots(figsize = (14, 10))
sns.heatmap(g, linewidths = 0.05, ax = ax)
ax.set_title('g')
ax.invert_yaxis()
f.savefig('g.jpg', dpi=100, bbox_inches='tight') 
 '''
    ...
    for i in range(32):
        if (i == 31):
            ...
            dl = np.sqrt((FS_x[i]-FS_x[i+1])**2+(FS_y[i]-FS_y
                                                 [i+1])**2)
        kernel_p_out[j] = 1/(2*np.pi)**2*dl*g[j][i]**2 / \
            1 + kernel_p_out[j]
x_new = np.linspace(0, 32, 200)
...
''' f, ax = plt.subplots(figsize=(14, 10))
sns.heatmap(kernel[:, :, int(num_freq(freq_cut, temp)/2)],
...
''' f, ax= plt.subplots(figsize = (14, 10))
sns.heatmap(Gap_function_in, linewidths = 0.05, ax = ax)
ax.set_title('g')
ax.invert_yaxis() '''

# Gap_function_in = 2*np.random.rand(32, num_freq(freq_cut, temp))
convoluted_function = np.ones((32, num_freq(freq_cut, temp), num_freq(freq_cut, temp)))

for k in range(32):
    for i in range(max_iter):
        for j in range(num_freq(freq_cut, temp)):
            convoluted_function[k, j, :] = (Gap_function_in[k, :]-mf_list1/mf_list1[j]
                                         * Gap_function_in[k][j])/np.sqrt(mf_list1**2 + Gap_function_in[k, :]**2)
            Gap_function_out[k][j] = np.pi*temp * \
                np.dot(convoluted_function[k, j, :], kernel[k, j, :])
        diff = np.linalg.norm(
            Gap_function_in[k, :] - Gap_function_out[k, :])
        Gap_function_in[k, :] = Gap_function_out[k, :]
        if (diff < tol):
            # print('\nat i = ', i, 'p = ', k,
            # 'times, the convergence is achieved')
            break
...
''' def myplot(matrix1):
    ...
    ax1.plot_surface(xs, ys, z1, color="blue", alpha=0.5, cmap='gnuplot',
